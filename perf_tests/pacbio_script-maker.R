...
#'
#' This file creates the file `pacbio_scripts.sh` that has each iteration of
#' both programs (with 1, 10, and 100 thousand reads, plus 1 and 4 threads for jackalope)
...
commands = list(simlord = "./pacbio.sh",
                jackalope = "Rscript ./pacbio.R")

n_reps <- 10

cmd_info <- rbind(expand.grid(prog = "simlord", nreads = c(1, 10, 100), threads = 1,
                             rep = 1:n_reps),
...
scripts <- sapply(1:nrow(cmd_info), function(i) {
    ...
    .fn <- sprintf("pacbio_out/runs/%s_%ikreads_%ithreads_rep%i.out", cmd_info$prog[i],
