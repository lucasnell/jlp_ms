...
```{r illumina-prof_pred_sourceCpp}
...
reference genome consisting of 4 sequences, each containing only a single nucleotide.
Then I simulated 100-bp reads from the HiSeq 2500 platform ten times.
Reads were matched with the sequence they derived from based on which nucleotide
comprises the majority of the read.
...
```{r illumina-test_mm_qual_plots1, fig.height=3, fig.width=5, fig.cap=ill_plot1_cap}
# Should be ~0.25 each:
map_dfr(1:10, function(i) tibble(x = 1:4, y = map_dbl(1:4, ~ mean(nts[[i]] == .x)))) %>% 
    mutate(x = factor(x, levels = 1:4, labels = rg$names())) %>% 
    ggplot() + 
...
```{r illumina_test_mm_qual_plots2_cap}
...
```{r illumina-test_mm_qual_plots2, fig.width=5.5, fig.height=5, fig.cap=ill_plot2_cap}
#'
...
# saveRDS(qbp_df, "qbp_df.rds")
qbp_df <- readRDS("qbp_df.rds")
...
```{r illumina-test_mm_qual_plots3_cap}
...
```{r illumina-test_mm_qual_plots3, fig.width=4.25, fig.height=4, fig.cap=ill_plot3_cap}
#'
...
mmbq_df <- map_dfr(1:10,
                   function(i) {
                       tibble(mm = mm_by_qual(reads[[i]], quals[[i]], max_qual = 50) %>%
                                  keep(~ . > 0),
                              qual = sort(unique(qbp_all[,i])),
                              rep = i) %>%
...
mmbq_df %>%
    ...
    scale_x_continuous("Predicted mismatch count", breaks = log(10^(1:3)),
                       labels = 10^(1:3), 
                       limits = log(range(c(mmbq_df$mm, mmbq_df$mm_pred)))) +
...
indels occurs that does not depend on the position on the read.
I am testing the accuracy of the output from `illumina` by starting with a
...
The predicted number of reads with indels was simply the product of the indel rate,
read length, and the number of reads.



```{r illumina-fake_ill_prof}
...
read_tsv(paste0(dir, "test_prof.txt"))
```



```{r illumina-indel_rates, fig.height=4, fig.width=4}
...
```{r illumina-indel_rates_plot, fig.height=4, fig.width=4, fig.cap=indel_rates_plot_cap}
