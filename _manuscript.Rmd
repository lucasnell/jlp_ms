---
title: "jackelope: a swift, light-weight phylogenomic and high-throughput sequencing simulator"
...
In the present paper I introduce `jackelope`, the first available high-throughput
...
Lastly, I demonstrate the usefulness of `jackelope` in informing study design
...
All inputs and outputs files can optionally be read or written with `gzip` compression.
...
`create_variants` function, and is created using the `create_mevo` function.
...
A table of rates from `r citet("10.1534/g3.116.030890")` is included in the package
as a guide to users.
...
inputs in the `create_mevo` function.
...
Function `create_mevo` also requires information about the relative rates of
...
particularly those focusing on population genomics or phylogenetics.
