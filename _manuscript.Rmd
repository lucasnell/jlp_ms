...
<!-- methods -->
Users can view the number of chromosomes, chromosome sizes, chromosome names,
individual chromosome strings, and nucleotide proportions (GC or otherwise).
They can also edit chromosome names, remove one or more chromosomes by name,
and add chromosomes manually.
All `N`s in the reference genome can be replaced with nucleotides
that are sampled with weights proportional to their equilibrium distributions
(provided by the user).
If the reference genome contains scaffolds or contigs, you can filter them by size or
merge them into one.
<!-- write -->
Reference genomes can be written to FASTA files.



The second class (`variants`) stores haploid variant information.
It does not store whole variant genomes, but
rather just their mutation information---this dramatically reduces memory usage.
<!-- methods -->
The user can view the number of chromosomes/variants,
variant chromosome sizes, chromosome/variant names, and individual
variant-chromosome strings.
Users can also edit variant names, remove one or more variants by name,
add blank variants, duplicate existing variants, and manually add mutations.
<!-- write -->
Variant information can be written to VCF files, where each variant can optionally be
considered one of multiple haplotypes for samples with ploidy levels > 1.
...
for various aspects of the molecular evolution (substitutions, indels [insertions and deletions], and among-site
...
For the phylogenomic methods, variants are simulated along tree branches 
using the Doob--Gillespie algorithm [@Doob_1942; @Gillespie_1976].
I used this method because it only requires instantaneous rates rather than the
calculation of a transition-probability matrix, which makes it easily used
for indels [@Yang_2006].
It starts by using the reference genome sequences as those for the root of the tree.
...
When a mutation occurs, a position on the sequence and a mutation type are then sampled
(see following paragraphs for more on these processes).
Waiting times are generated until the sum of all times is greater than the
branch length.
...
The sequence position for a mutation is sampled with a probability proportional
to the mutation rate for that position.
A position's mutation rate is the product of the overall mutation rate
for the nucleotide at that position
(determined by the summed indel and substitution rates)
and its "gamma distance."
Gamma distances are mutation-rate modifiers that vary by position on the sequence,
and they are used to generate among-site variability in mutation rates.
...
<!---
Mutation type sampling
-->


A mutation type is sampled with probabilities proportional to the rate of each
mutation type for the nucleotide present at the sampled position.
...
I then use alias sampling [@Kronmal_1979; @Walker_1974] to sample within this vector.
...
Both reference genomes and variants can be input to the sequencing functions
for Illumina and PacBio sequencing.
