...
This can greatly reduce memory usage (unless mutation density is high), and it provides
the ability to output VCF files based on variants.
...
The remaining three methods use phylogenetic tree(s).
...
calculating the transition-probability matrix for each Gamma category based on
the branch length.
Substitutions are then sampled for each site using the matrix that coincides with
...
In the text, numbers presented are means from these 5 tests unless otherwise stated.
...
`jackalope` averaged `r sprintf("%.1f", ill_mb$jackalope)` MB used for Illumina reads and
...
I lastly compared the performance of `jackalope` to that of `NGSphy` [@Escalona_2018]
...
`INDELible`.
I did the same for the last step, except that the haploid variants were used for
sequencing reads instead of a reference genome.
...
one_run <- function(gsize_, mdepth_, nreads_) {
    ...
    # Some combos aren't done yet
    if (!file.exists(fn)) {
        return(tibble(prog = paste(c("ngsphy", "jackalope"), rep(c(1,4),each=2)),
                      gsize = gsize_, mdepth = mdepth_, nreads = nreads_,
                      sec = NA_real_, Mb = NA_real_))
    }
    lines <- readLines(fn)
...
pip_runs <- expand.grid(gsize_ = c(2, 20), 
    ...
    mutate(gsize = factor(gsize, levels = sort(unique(gsize)),
                          labels = paste(sort(unique(gsize)), "Mb")),
           nreads = factor(nreads, levels = sort(unique(nreads)),
                           labels = c("100k", "1M", "10M")),
...
PUT DESCRIPTION OF COMPARISON HERE.


ALSO, ALL CODE BELOW THIS IS NOT VETTED AT ALL!
...
var_plot_cap <- paste("Performance comparison between \\texttt{jackalope} and",
                      "\\texttt{NGSphy} in generating variants from a",
                      "reference genome, then producing Illumina reads from variants.",
                      "Sub-panel columns indicate the size of the genome from which",
                      "variants were generated.",
                      "Sub-panel rows indicate the maximum tree depth of the phylogeny",
                      "used for the simulations.",
...
# pip_time <- 
pip_runs %>%
    filter(prog != "ngsphy 4",  # this had no effect, so I'm not plotting it
           !is.na(sec)) %>%
    ggplot(aes(nreads, sec)) +
    geom_hline(yintercept = 0, linetype = 2, color = "gray70") +
    geom_jitter(aes(fill = prog), height = 0, width = 0.25, 
                shape = 21, color = "gray40") +
    # scale_y_continuous("Elapsed time (sec)", limits = c(0, NA)) +
...
# pip_plot <- plot_grid(pip_time, pip_mem, ncol = 1)

ggsave("Fig_4.pdf", pip_time, width = 88, height = 88, units = "mm")
```
