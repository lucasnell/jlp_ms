...
```{r variants-perf-test-inputs-caption, echo = FALSE, eval = TRUE}
var_plot_cap <- paste("Performance comparison between \\texttt{jackalope} and",
                      "\\texttt{Seq-Gen} in generating variants from a",
                      "reference genome. In the legend, numbers inside parentheses",
                      "indicate the number of threads used.")
```

```{r variants-perf-test-inputs, echo = FALSE, eval = TRUE, fig.width=80/25.4, fig.height=120/25.4, fig.cap=var_plot_cap}
...
var_runs <- rbind(expand.grid(prog_ = "seqgen", gsize_ = c(2, 20, 200), threads_ = 1,
    ...
    mutate(gsize = factor(gsize, levels = sort(unique(gsize)),
           ...
           type = factor(type, levels = c("jackalope__1", "jackalope__4", "seqgen__1"),
                         labels = c("jackalope (1)", "jackalope (4)", 
                                    "Seq-Gen (1)")))
...
var_time <- var_runs %>% 
    ...
    facet_wrap(~ gsize, scales = "free_y") +
    theme(axis.title.y = element_text(size = 11),
          ...
          legend.position = "top",
          legend.text = element_text(size = 9, margin = margin(0,0,0, l = -8)),
...
var_legend <- get_legend(var_time)
...
var_plot <- plot_grid(var_legend, plot_grid(var_time, var_mem, ncol = 1,
                                                  labels = letters[1:2]),
                         rel_heights = c(0.1, 1), ncol = 1)

ggsave("figs/Fig_2.pdf", var_plot, width = 80, height = 120, units = "mm")
...
For Illumina read simulation, I compared `jackalope` to `ART`
`r citep("10.1093/bioinformatics/btr708")`.
The tests consisted of reading a 2 Mb genome from a FASTA file, simulating
$2 \times 150$ reads from the HiSeq 2500 platform, and writing them to FASTQ files.
There were separate tests for 100 thousand, 1 million, and 10 million total reads.
`jackalope` was tested for 1 and 4 threads, but
`ART` could only be tested for 1.



```{r variants-perf-test-inputs-caption, echo = FALSE, eval = TRUE}
ill_plot_cap <- paste("Performance comparison between \\texttt{jackalope} and",
                      "\\texttt{ART} in generating Illumina reads.",
                      "In the legend, numbers inside parentheses",
                      "indicate the number of threads used.")
```

```{r illumina-perf-test-inputs, echo = FALSE, eval = TRUE, fig.width=80/25.4, fig.height=120/25.4, fig.cap=ill_plot_cap}
...
ill_runs <- bind_rows(crossing(prog = "art", nreads = c(0.1, 1, 10), threads = 1,
    ...
    mutate(nreads = factor(nreads, levels = sort(unique(nreads))),
           type = paste(prog, threads, sep = "__"),
           type = factor(type, levels = c("jackalope__1", "jackalope__4", "art__1"),
                         labels = c("jackalope (1)", "jackalope (4)", 
                                    "ART (1)")))
...
ill_time <- ill_runs %>%
    ...
    geom_jitter(aes(fill = type), height = 0, width = 0.25, 
                shape = 21, color = "gray40") +
    scale_y_continuous("Time taken (sec)", limits = c(0, NA)) +
    facet_wrap(~ nreads, scales = "free_y") +
    theme(axis.title.y = element_text(size = 11),
          axis.text.y = element_text(size = 8),
          axis.title.x = element_blank(),
          axis.ticks.x = element_blank(),
...
ill_legend <- get_legend(ill_time)
...
ill_plot <- plot_grid(ill_legend, plot_grid(ill_time, ill_mem, ncol = 1,
                                                  labels = letters[1:2]),
                         rel_heights = c(0.1, 1), ncol = 1)

# ggsave("figs/Fig_3.pdf", ill_plot, width = 80, height = 120, units = "mm")
...
I compared PacBio read simulation to SimLoRD `r citep("10.1093/bioinformatics/btw286")`.
I tested them by having them read a 2 Mb genome from a FASTA file, simulate
PacBio reads using default parameters, and writing them to FASTQ files.
There were separate tests for 1, 10 and 100 thousand total reads.
`jackalope` was tested for 1 and 4 threads, but
`SimLoRD` could only be tested for 1.


```{r pacbio-perf-test-inputs-caption, echo = FALSE, eval = TRUE}
pb_plot_cap <- paste("Performance comparison between \\texttt{jackalope} and",
                      "\\texttt{SimLoRD} in generating PacBio reads.",
                      "In the legend, numbers inside parentheses",
                      "indicate the number of threads used.")
```

```{r illumina-perf-test-inputs, echo = FALSE, eval = TRUE, fig.width=80/25.4, fig.height=120/25.4, fig.cap=pb_plot_cap}
...
pb_runs <- bind_rows(crossing(prog = "simlord", nreads = c(1, 10, 100), threads = 1,
    ...
    mutate(nreads = factor(nreads, levels = sort(unique(nreads))),
           type = paste(prog, threads, sep = "__"),
           type = factor(type, levels = c("jackalope__1", "jackalope__4", "simlord__1"),
                         labels = c("jackalope (1)", "jackalope (4)", 
                                    "SimLoRD (1)")))
...
pb_plot <- plot_grid(pb_legend, plot_grid(pb_time, pb_mem, ncol = 1,
                                                  labels = letters[1:2]),
                         rel_heights = c(0.1, 1), ncol = 1)

# ggsave("figs/Fig_4.pdf", pb_plot, width = 80, height = 120, units = "mm")
