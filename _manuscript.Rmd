...
`INDELible`, but I used a species tree instead of gene trees.
I used a species tree here because `NGSphy` requires a specialized database structure
for inputting gene trees that `jackalope` cannot read.
...
# pip_runs_cpu %>%
#     filter(cpu_sec > sec) %>%
#     select(prog, gsize, mdepth, nreads, sec, cpu_sec)
