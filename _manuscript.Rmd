...
For position $i$ inside region $j$, the overall mutation rate is $\mu_i \gamma_j$,
where $\mu_i$ is the mutation rate associated with the nucleotide at position $i$ and
...
region ($\sum_i \mu_i \gamma_j$ for region $j$ and all $i$ positions inside it).
...
The process evolves chromosomal sequences along phylogenetic trees, and 
...
(i.e., $\sum_j \sum_i \mu_i \gamma_j$ for all $j$ in the sequence and all $i$ in
the $j$th region).
...
Second, segregating sites from coalescent output can provide the variants and locations 
at which mutations occur.
The user provides information to sample mutation types at each segregating site.
...
ssites <- scrm(paste("40", ref$n_seqs(), "-t 10 -I 2 20 20 100"))
```
...
I also write the true variant information to a VCF file.
...
I also write the true phylogenetic tree to a NEWICK file.
