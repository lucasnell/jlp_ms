...
hap_plot_cap <- paste("Performance comparison between \\texttt{jackalope} and",
                      "\\texttt{INDELible} in generating variant haplotypes from a",
                      "reference genome.",
...
`INDELible`.
Because `NGSphy` cannot take `ms`-style gene trees, I simulated gene trees
using `SimPhy` [version 1.0.2; @Mallo_2015].
I first generated a random tree of 8 species with a maximum depth of 1, then 
used that in `SimPhy` to generate 10 gene trees for each of 20 loci.
I ran separate simulations where the average max tree depths among all gene trees 
was 0.1, 0.01, and 0.001.
I adjusted the tree-wide substitution rates to generate these 3 scenarios.
I passed the outputs from `SimPhy` directly to `NGSphy`, whereas I 
converted them to `ms`-style output files for `jackalope`.
I used the same parameters for the Illumina sequencing step as in the previous section,
except that the variant haplotypes were used for sequencing reads instead of
a reference genome.
