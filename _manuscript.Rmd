...
and greater maximum tree depth (Figure \@ref(fig:haplotypes-perf-test-plot)).
...
`r sprintf("%.1f", low_mut_mb$indelible$jackalope)` MB memory, 
while `INDELible` used `r sprintf("%.1f", low_mut_mb$indelible$indelible)` MB.
...
`r sprintf("%.1f", high_mut_mb$indelible$jackalope)` MB memory and `INDELible` used
`r sprintf("%.1f", high_mut_mb$indelible$indelible)` MB.
I chose to use 64-bit integers in `jackalope` to allow chromosomes to exceed
2 Gb in length.
Memory usage increased by only `r sprintf("%.1f", avg_diff_threads$indelible * 1e3)`kB 
when `jackalope` used 4 threads.
...
hap_p <- plot_grid({hap_time_seqgen + x + y_time} %>% no_f() %>% no_x(), 
                   ...
                   ncol = 2, align = "v",
                   labels = LETTERS[1:4], label_fontface = "plain")

save_fig("Fig_2.pdf", hap_p, width = 112, height = 112)
