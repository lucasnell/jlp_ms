...
difference was slightly less drastic when there were more mutations: 
for the larger genome size and greater maximum tree depth (Figure
\@ref(fig:haplotypes-perf-test-plot)B).
...
Calling for 4 threads resulted in `r sprintf("$%.2f \\times$", ngsphy_threads$sec)` 
...
`r sprintf("$%.1f \\times$", filter(pip_prop_mb, mdepth == 0.001, gsize == "20 Mb")$asmuch)`
and
`r sprintf("$%.1f \\times$", filter(pip_prop_mb, mdepth == 0.01, gsize == "20 Mb")$asmuch)`
as much memory, respectively.
...
save_fig("Fig_4.pdf", pip_plot, width = 169, height = 112)
```
