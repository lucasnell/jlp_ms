...
hap_runs <- expand.grid(gsize_ = c(2, 20), mdepth_ = c(0.001, 0.01, 0.1),
    ...
    mutate(gsize = factor(gsize, levels = sort(unique(gsize)),
           ...
           prog = factor(prog, levels = c("jackalope 4", "jackalope 1",
                                          "indelible", "seq-gen")),
           comp = factor(comp, levels = c("indelible", "seq-gen")))
...
# hap_plot <- plot_grid(hap_time, hap_mem, ncol = 1)


hap_time <- plot_grid(hap_time_seqgen, hap_time_indelible, ncol = 1, 
                      labels = LETTERS[1:2], label_fontface = "plain")


save_fig("Fig_2.pdf", hap_time, width = 112, height = 169)
...
```{r haplotypes-perf-test-plot, echo = FALSE, eval = TRUE, fig.width=88/25.4, fig.height=88/25.4, fig.cap=hap_plot_cap}
hap_time
```
