...
author:
    - name: Lucas A. Nell
      nums: "1"
affil:
    - name: Department of Integrative Biology, University of Wisconsin, Madison, WI, USA
      num: "1"
fontsize: 12pt
...
numbersections: true
raggedtoc: true
include-before: "Correspondence: lucas@lucasnell.com"
output: 
...
I first outline the methods used within `jackalope`.
...
There are two main classes in `jackalope`, one that stores information on reference 
genomes (`ref_genome`), the other that stores information on sets of variant 
...
be unfair.
I generated the random species tree using `ape::rcoal` [@Paradis_2018].
...
pip_plot_cap <- paste("(A) Speed and (B) memory usage comparison between",
                      ...
                      "Results from \\texttt{NGSphy} are not shown for a genome size",
                      "of 20 Mb and maximum tree depth of 0.1 because it exceeded",
                      "the available memory on the testing platform.")
```
...
Documentation for `jackalope` has been developed using R's standard framework, with
documents for each major class and function.
A longer vignette with example usage is also included.
In it, I give examples of basic `jackalope` usage, including generating reference genomes
and variant haplotypes.
I also outline examples of how `jackalope` can generate HTS data that can inform 
...
by running `vignette("jackalope-intro")` inside R after `jackalope` is installed,
and at 
<https://jackalope.lucasnell.com/articles/jackalope-intro>.
