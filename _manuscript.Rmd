...
[@Oneill_2014pcg].
I use alias sampling [@Kronmal_1979; @Walker_1974] for all weighted sampling except
for when sampling for mutation positions.
...
(`ref_genome`), the other storing information on sets of variants (`variants`).
...
Variants are nested vectors of mutation information, not entire variant genomes;
this dramatically reduces memory usage.
I chose `R6` classes because they allow for fields that cannot be manipulated from
outside the class; I made the pointers to C++ objects private to prevent users
from accidentally manipulating them.
...
minimizes the chances of copying of large objects in memory.
...
Haploid reference genomes can be generated from FASTA files, optionally with index files---created using `samtools faidx`---that speed up processing.
...
There are three main processes that `jackalope` simulates to generate haploid variants
from a reference genome.
These processes inform mutation types, positions, and quantities.
There are also five different methods of simulating variants, and not all of these
methods use all underlying processes.
The first three sections below describe the three processes.
The last section outlines the five methods and which processes they use.
...
Mutations can be substitutions or indels (insertions / deletions).
A table of rates from  @Sung_2016 is included in the package as a guide for
reasonable rates to use.
...
The above information is used to create 4 vectors (one for each nucleotide),
each containing the rates of all possible mutation types:
substitutions to other nucleotides and insertions/deletions of all specified lengths.
After determining the position of the mutation (from the second process or from
input data),
I sample for a mutation type with weights proportional to the rate of each
mutation type for the nucleotide present at the mutation position.
...
The chromosomal position for a mutation is sampled with a probability proportional
to the mutation rate for that position.
The first factor determining a position's mutation rate is the summed indel and
substitution rates for the nucleotide at that position.
The second, optional factor is among-site variability in mutation rates.
In `jackalope`, this is incorporated using a fixed-sites model [@Yang_2002],
where different regions of each chromosome can have different rates of mutation.
For each region, this takes the form of a "gamma distance" that modifies the
nucleotide-associated mutation rates for positions within its region.
For position $i$ inside region $j$, the overall mutation rate is $\zeta_i \gamma_j$,
where $\zeta_i$ is the mutation rate associated with the nucleotide at position $i$ and
$\gamma_j$ is the gamma distance for region $j$.
Gamma distances can be generated from a Gamma distribution (with a fixed mean of 1)
for regions of uniform size.
Alternatively, both gamma distances and region sizes can be passed manually.


To perform this sampling, `jackalope` first sums the overall mutation rates for each
region ($\sum_i \zeta_i \gamma_j$ for region $j$ and all $i$ positions inside it).
It then uses these total rates as weights to sample regions;
I incorporated a binary search tree to speed this sampling.
Simple inversion sampling is used to sample within a region, and weights here 
are simply the nucleotide-associated mutation rates.
...
Mutation quantities are sampled when phylogenomic methods are employed.
I used the Doob--Gillespie algorithm [@Doob_1942; @Gillespie_1976] because
it only requires instantaneous rates rather than the
calculation of a transition-probability matrix, making it easily used
for indels [@Yang_2006].
The process evolves chromosomal sequences along simple phylogenetic trees, and 
the number of mutations occurring along a particular tree branch is proportional to
the branch's length.
All chromosomes evolve independently, and each chromosome can optionally use a
different phylogenetic tree.
If recombination is included, multiple phylogenetic trees ("gene trees") are used
per chromosome, and chromosomal regions referred to by different gene trees
evolve independently.
For the next paragraph, "sequence" refers to a region of DNA that evolves 
independently: a whole chromosome in the case of one phylogenetic tree,
or a chromosomal region in the case of gene trees.


Evolving sequences along phylogenetic trees starts by using the reference genome
sequences as those for the root of the tree.
Then, for each sequence and branch, waiting times are generated that represent
the time until the next mutation occurs somewhere on the sequence.
Wait times are generated from an exponential distribution with a rate equal to the
sum of mutation rates for all nucleotides in the sequence.
Waiting times (and resulting mutations) are generated until the sum of all times
is greater than the branch length.
The resulting sequence is then assigned to the daughter node on the tree.
...
information on all the methods can be found in the `vars_functions` documentation.
First, a variant call format (VCF) file can directly specify mutations for each variant.
This method works using the `vcfR` package 
[@Knaus_2017] and
is the only method that does not require any mutation-type information.
Second, segregating sites from coalescent output can provide the locations of mutations.
...
For this method, the user must provide mutation-type information only.


The remaining methods involve phylogenetic tree(s) that inform mutation quantities,
and require information on mutation types and positions from the user.
In the third method, phylogenetic tree(s) can be directly input from either
`phylo` object(s) or NEWICK file(s)---either one tree for all chromosomes or one
per chromosome.
...
The gamma distances for among-site heterogeneity are optionally saved as a BED file.
...
[@Rambaut_1997].
I chose `Seq-Gen` because it is used by `TreeToReads` and because it can generate
...
gene trees produced by the commandline version of `scrm`.

```{bash scrm-test, echo = FALSE}
...
ggsave("Fig_2.pdf", var_plot, width = 112, height = 112, units = "mm")
```
...
`ART` [@Huang_2011] and
`SimLoRD` [@St_cker_2016], respectively.
...
```{r sequencing-perf-test-plot-make, echo = FALSE, eval = TRUE}
...
For mutation-type information, I used the TN93 substitution model,
...
I first used the `scrm` package [@Paul_R._Staab_2015]
...
and the population-scaled mutation rate ($\theta = 4 N_0 \mu$) is quite high (`1000`).
The `scrm` command for this is `40 5 -t 1000 -I 2 20 20 100`.
...
I also write the true variant information to VCF file `variants.vcf`.

```{r examples-divergence-create-for-show}
vars <- create_variants(ref, vars_ssites(ssites), sub, ins, del)
write_vcf(vars, "variants")
```
```{r examples-divergence-create, eval = TRUE, echo = FALSE}
vars <- create_variants(ref, vars_ssites(ssites), sub, ins, del)
```
...
In this case, each individual variant's reads are output to separate FASTQ files.
...
including it with the mutation-type information allows me to create variants based
on this phylogeny:
...
`24 1 -T -r 1 1000 -I 3 8 8 8 0 -ej 1.0 3 1 -ej 0.5 2 1` for a chromosome size of 1000), producing the object `gtrees`.
...
The version used in this manuscript was 0.1.3.
