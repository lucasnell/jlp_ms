...
# Color palette for most performance testing plots:
perf_palette <- magma(5, begin = 0, end = 0.85)[c(1, 4, 5)]
# For Figure 2 bc it needs 4 colors:
perf_palette4 <- magma(7, begin = 0, end = 0.85)[c(1, 4, 6, 7)]
...
# height and width should be in mm
...
(i\) sets of variant haplotypes from a reference genome and
...
phylogenetic tree using `INDELible` [@Fletcher_2009]
and generates Illumina reads using `ART` [@Huang_2011].
The newer pipeline `NGSphy` [@Escalona_2018] also uses `INDELible` and `ART`, and
...
(`ref_genome`), the other that stores information on sets of variant 
haplotypes (`haplotypes`), each set based on one reference.
...
go_cap <- paste("Overview of primary \\texttt{jackalope} functions, classes,",
                "inputs, and outputs.",
                "Circles \\texttt{sub\\_models} and \\texttt{haps\\_functions}",
...
Information on all the methods can be found in the documentation (under `haps_functions`).
...
a `phylo` object or a NEWICK file.
...
All mutation models in `jackalope` are finite-site models.
...
compared to infinite-site models such as those used in `fastsimcoal` [@Excoffier_2011].
...
the Doob--Gillespie algorithm [@Doob_1942; @Gillespie_1976], optionally 
with the $\tau$-leaping approximation [@Wieder_2011; @Cao_2006].
...
It has a default value of 0.03, as recommended by @Cao_2006, but can be changed
by the user.
...
overall read-generation procedure from the `ART` program [@Huang_2011].
PacBio read simulation is similarly based on `SimLoRD` [@St_cker_2016].
...
I validated `jackalope` by conducting a series of simulations for variant haplotype
...
indels at the same rate (Supporting Information Figures S14).
Performance increased with $\epsilon$, except for when simulating only deletions
(Supporting Information Figures S15).
...
when only simulating deletions (Supporting Information Figures S14).
...
For creating variant haplotypes, I compared `jackalope` to 
`Seq-Gen` [version 1.3.4; @Rambaut_1997] and
`INDELible` [version 1.03; @Fletcher_2009].
I chose `Seq-Gen` because it is widely used and can generate sequences based on
gene trees. 
It is relatively simple and written in C, so it should provide a conservative
estimate of how well `jackalope` performs against similar programs.
...
gene trees produced by the command line version of `scrm`.
...
No indels were included when comparing to `Seq-Gen`.
For the comparison to `INDELible`, the total rate for insertions was 0.1 per site 
...
The exact Doob--Gillespie algorithm was used for indels because `INDELible` does
not use the approximation.
Output was written to FASTA files for all tests.
`jackalope` was tested for 1 and 4 threads, but
`Seq-Gen` and `INDELible` for only 1 because they cannot use multiple threads.
...
```{r haplotypes-perf-test-data, echo = FALSE, eval = TRUE}

one_run <- function(gsize_, mdepth_, prog_) {
    # gsize_ = 20; mdepth_ = 0.1; prog_ = "indelible"; ## rm(gsize_, mdepth_)
    fn <- sprintf("perf_tests/%s/results/%iMb__%.4f__times.txt",
                  prog_, gsize_, mdepth_)
    lines <- readLines(fn)
...
hap_runs <- expand.grid(gsize_ = c(2, 20), mdepth_ = c(0.001, 0.01, 0.1),
                        prog_ = c("indelible", "seq-gen")) %>% 
    as.list() %>% 
    {pmap_dfr(., one_run)} %>% 
    mutate(gsize = factor(gsize, levels = sort(unique(gsize)),
           ...
           prog = factor(prog, levels = c("jackalope 4", "jackalope 1",
                                          "seq-gen", "indelible")),
           comp = factor(comp, levels = c("seq-gen", "indelible")))

# # To look at the exact numbers:
# hap_runs %>%
#     group_by(comp, gsize, mdepth, prog) %>%
...
low_mut_mb <- map(c("indelible", "seq-gen"), function(.c) {
  hap_runs %>%
    filter(comp == .c, gsize == "2 Mb", mdepth == 0.001) %>%
...
high_mut_mb <- map(c("indelible", "seq-gen"), function(.c) {
    hap_runs %>%
    filter(comp == .c, gsize == "20 Mb", mdepth == 0.1) %>%
    mutate(prog = gsub(" 1| 4", "", paste(prog))) %>% 
    group_by(prog) %>% 
    summarize(MB = mean(MB)) %>% 
    spread("prog", "MB") %>% 
...
names(low_mut_mb) <- names(high_mut_mb) <- c("indelible", "seq-gen")


# Average difference between jackalope with 1 vs 4 threads:
avg_diff_threads <- hap_runs %>%
    group_by(comp, gsize, mdepth, prog) %>%
    summarize(MB = mean(MB)) %>% 
    group_by(comp, gsize, mdepth) %>%
...
`jackalope` using 4 threads was faster than `Seq-Gen` but
slightly slower when using only 1 thread;
`jackalope` performed best at low maximum tree depths in the 20 Mb genome
...
For the smaller 2 Mb genome, memory usage was greater in `jackalope` than both
`Seq-Gen` and `INDELible` (Figure \@ref(fig:haplotypes-perf-test-plot)C--D).
...
For the larger 20 Mb genome, whether `jackalope` used more memory than `Seq-Gen`
depended on the maximum tree depth  (Figure \@ref(fig:haplotypes-perf-test-plot)C).
This is because `jackalope` stores one copy of reference sequences plus information
for each mutation, whereas `Seq-Gen` stores a copy of each haplotype's sequences.
At very high mutation densities, storing all the separate sequences is more efficient.
Memory usage was largely the same between `jackalope` and `INDELible` for the 20 Mb
genome.
The slightly higher high relative usage of `jackalope` at the maximum tree depth of
0.1 was likely due to `jackalope` using 64-bit integers to store positions 
instead of 32-bit.
...
```{r haplotypes-perf-test-plot-caption, echo = FALSE, eval = TRUE}
hap_plot_cap <- paste("Performance comparison between \\texttt{jackalope} and",
                      "(A,C) \\texttt{Seq-Gen} and (B,D) \\texttt{INDELible}",
                      "in generating variant haplotypes from a reference genome,",
                      "for varying maximum gene tree depths.",
                      "Panels A and B show elapsed time, and panels C and D show",
                      "maximum memory used during operation.",
                      "Sub-panel rows indicate the size of the genome from which",
                      "haplotypes were generated.",
                      "Superscripts in the labels indicate",
                      "the number of threads used.")
```

```{r haplotypes-perf-test-plot-make, echo = FALSE, eval = TRUE}
...
hap_time_seqgen <- hap_runs %>%
    filter(comp == "seq-gen") %>%
    group_by(prog, gsize, mdepth) %>% 
    summarize(sec = mean(sec)) %>% 
    ungroup() %>% 
    ggplot(aes(log10(mdepth), sec)) +
    geom_hline(yintercept = 0, linetype = 2, color = "gray70") +
    geom_jitter(data = hap_runs %>% filter(comp == "seq-gen"),
                aes(color = prog), shape = 1, size = 2, width = 0.2) +
    geom_line(aes(color = prog), size = 0.75) +
    geom_text(data = tibble(prog = "seq-gen",
                            label = "bold(Seq-Gen^1)",
                            sec = 14, gsize = "20 Mb", mdepth = 0.001),
              aes(label = label, color = prog),
              hjust = 0, vjust = 0, size = 10 / 2.835, parse = TRUE) +
    geom_text(data = tibble(prog = c("jackalope 4", "jackalope 1"),
                            label = c("bold(jackalope^4)", "bold(jackalope^1)"), 
                            sec = c(0.8, 2.25), 
                            gsize = "2 Mb", mdepth = 0.001),
              aes(label = label, color = prog),
              hjust = 0, vjust = 1, size = 10 / 2.835, parse = TRUE) +
    facet_grid(gsize ~ ., scales = "free") +
    theme(axis.title = element_text(size = 11),
          axis.text = element_text(size = 8),
          plot.margin = margin(0,0,0,0),
          strip.text = element_text(size = 10),
          strip.background = element_blank()) +
    scale_color_manual(NULL, guide = FALSE, 
                       values = perf_palette4[1:3]) +
    NULL



hap_time_indelible <- hap_runs %>%
    filter(comp == "indelible") %>%
    group_by(prog, gsize, mdepth) %>% 
    summarize(sec = mean(sec)) %>% 
    ungroup() %>% 
    ggplot(aes(log10(mdepth), sec)) +
    geom_hline(yintercept = 0, linetype = 2, color = "gray70") +
    geom_jitter(data = hap_runs %>% filter(comp == "indelible"),
                aes(color = prog), shape = 1, size = 2, width = 0.2) +
    geom_line(aes(color = prog), size = 0.75) +
    geom_text(data = tibble(gsize = "20 Mb",
                            prog = "indelible",
                            label = "bold(INDELible^1)",
                            mdepth = 0.001,
                            sec = 255),
              aes(label = label, color = prog),
              hjust = 0, vjust = 0, size = 10 / 2.835, parse = TRUE) +
    ...
    theme(axis.title = element_text(size = 11),
          ...
          strip.background = element_blank()) +
    scale_color_manual(NULL, guide = FALSE, 
                       values = perf_palette4[c(1:2,4)]) +
    NULL


hap_mb_seqgen <- hap_runs %>% 
    filter(comp == "seq-gen") %>%
    group_by(comp, prog, gsize, mdepth) %>% 
    summarize(MB = mean(MB)) %>% 
    ungroup() %>% 
    ggplot(aes(log10(mdepth), MB)) +
    geom_hline(yintercept = 0, linetype = 2, color = "gray70") +
    geom_jitter(data = hap_runs %>% filter(comp == "seq-gen"),
...
no_f <- function(p) p + theme(strip.text = element_blank())
y_break_fun <- function(x) {
    m <- max(x)
    if (m > 400) return(c(0, 200, 400))
    if (m > 290) return(c(0, 150, 300))
    if (m > 100) return(c(0, 40, 80))
    if (m > 20) return(c(0, 12, 24))
    if (m > 15) return(c(0, 8, 16))
    return(c(0, 0.8, 1.6))
}
x <- scale_x_continuous("Maximum tree depth",
                        breaks = -3:-1, labels = c(0.001, 0.01, 0.1))
y_time <- scale_y_continuous("Elapsed time (sec)", breaks = y_break_fun, 
                             limits = c(0, NA))
y_mb <- scale_y_continuous("Memory used (MB)", breaks = y_break_fun, 
                           limits = c(0, NA))

hap_p <- plot_grid({hap_time_seqgen + x + y_time} %>% no_f() %>% no_x(), 
                   {hap_time_indelible + x + y_time} %>% no_y() %>% no_x(), 
                   NULL, NULL,
                   {hap_mb_seqgen + x + y_mb} %>% no_f(),
                   {hap_mb_indelible + x + y_mb} %>% no_y(), 
                   ncol = 2, align = "v",
                   labels = c(LETTERS[1:2], "", "", LETTERS[3:4]), 
                   label_fontface = "plain",
                   rel_heights = c(1, 0.2, 1))

save_fig("Fig_2.pdf", hap_p, width = 112, height = 140)
...
`ART` has the added benefit of being used for both `NGSphy` and `TreeToReads`.
...
`jackalope` outperformed both `ART` and `SimLoRD` in terms of speed
...
It increased `r sprintf("%.1g", ill_reads_mb$jackalope)` MB across the range of 
Illumina reads and `r sprintf("%.1g", pb_reads_mb$jackalope)` MB across the PacBio range.
...
save_fig("Fig_3.pdf", seq_plot, width = 112, height = 112)
...
`INDELible`, except that I used a species tree instead of gene trees.
I am using a species tree because the method of using gene trees in `NGSphy` requires
the use of `SimPhy` [@Mallo_2015] to both conduct coalescent simulations and 
create variant haplotypes.
Because it includes an extra step, the performance comparison to `jackalope` would
be unfair.
I generated the random species tree using `ape::rcoal(10)` [@Paradis_2018].
...
pip_prop_mb <- pip_runs %>%
    ...
    summarize(asmuch = MB[prog == "ngsphy"] / MB[prog == "jackalope"]) %>% 
    ungroup()


# X as much between NGSphy with 1 and 4 threads
...
# X as much extra memory for 10M reads vs 100k
pip_reads_mb <- pip_runs %>%
    mutate(prog = gsub(" 1| 4", "", paste(prog))) %>%
    filter(nreads %in% c(0.1, 10)) %>% 
    group_by(prog, gsize, mdepth) %>%
    summarize(asmuch = mean(MB[nreads == 10]) / mean(MB[nreads == 0.1])) %>% 
    group_by(prog) %>% 
    summarize(asmuch = mean(asmuch)) %>% 
    ungroup() %>% 
    spread("prog", "asmuch") %>%
    as.list()
...
pip_runs_p <- pip_runs %>%
    ...
    mutate(prog = paste(prog),
           ...
           prog = factor(prog, levels = c("jackalope 4", "jackalope 1", "ngsphy"))) %>%
    identity()
...
The speed advantage of `jackalope` was greatest at low maximum tree depths.
...
Calling for 4 threads resulted in `r sprintf("$%.3f \\times$", ngsphy_threads$sec)` 
...
`r sprintf("$%.1f \\times$", filter(pip_prop_mb, mdepth == 0.1, gsize == "2 Mb")$asmuch)`
as much memory.
`NGSphy` could not be run for a 20 Mb genome at a maximum tree depth of 0.1 because
...
100k reads, `jackalope` consumed `r sprintf("$%.4f \\times$", abs(pip_reads_mb$jackalope))`
as much memory, and `NGSphy` used `r sprintf("$%.4f \\times$", abs(pip_reads_mb$ngsphy))` 
as much.
...
pip_time <- pip_runs_p %>%
    ...
    geom_text(data = tibble(mdepth = 0.001,
                            ...
                            label = c("bold(jackalope^4)", "bold(jackalope^1)",
                                      "bold(NGSphy^{1 * '*'})"),
                            log_reads = c(7.3, 6.8, 6.6),
                            sec = c(-5, 175, 500)),
              aes(label = label, color = prog),
              hjust = 1, vjust = 1, size = 10 / 2.835, parse = TRUE) +
    scale_y_continuous("Elapsed time (min)", limits = c(-15 / 60, NA), 
...
# # ==========================================
# # In case you're interested in looking at proportions in relation to NGSphy:
# # ==========================================
# 
# pip_runs_p %>%
#     group_by(prog, gsize, mdepth, nreads) %>%
#     summarize(sec = mean(sec)) %>%
#     group_by(gsize, mdepth, nreads) %>%
#     summarize(sec1 = mean(sec[prog == "jackalope 1"]) / mean(sec[prog == "ngsphy"]),
#               sec4 = mean(sec[prog == "jackalope 4"]) / mean(sec[prog == "ngsphy"])) %>%
#     ungroup() %>%
#     ggplot(aes(log10(nreads * 1e6))) +
#     # geom_hline(yintercept = 1, linetype = 2, color = "gray70") +
#     geom_line(aes(y = sec1), size = 0.75, color = "dodgerblue") +
#     geom_point(aes(y = sec1), size = 2, color = "dodgerblue") +
#     geom_line(aes(y = sec4), size = 0.75, color = "firebrick") +
#     geom_point(aes(y = sec4), size = 2, color = "firebrick") +
#     scale_x_continuous("Millions of reads",
#                        breaks = log10(c(0.1e6, 1e6, 10e6)), labels = c(0.1, 1, 10)) +
#     scale_y_continuous("Relative elapsed time") +
#     facet_grid(gsize ~ mdepth, scales = "free_y") +
#     theme(axis.title = element_text(size = 11),
#           axis.text = element_text(size = 8),
#           plot.margin = margin(0, 0, l = 6, b = 6),
...
#     summarize(MB = mean(MB)) %>% 
#     group_by(gsize, mdepth) %>% 
#     summarize(MB1 = mean(MB[prog == "jackalope 1"]) / mean(MB[prog == "ngsphy"]),
#               MB4 = mean(MB[prog == "jackalope 4"]) / mean(MB[prog == "ngsphy"])) %>% 
#     ungroup() %>% 
#     ggplot(aes(log10(mdepth))) +
#     # geom_hline(yintercept = 1, linetype = 2, color = "gray70") +
#     geom_line(aes(y = MB1), size = 0.75, color = "dodgerblue") +
#     geom_point(aes(y = MB1), size = 2, color = "dodgerblue") +
#     geom_line(aes(y = MB4), size = 0.75, color = "firebrick") +
#     geom_point(aes(y = MB4), size = 2, color = "firebrick") +
#     scale_y_continuous("Relative memory used", limits = c(0, NA)) +
#     scale_x_continuous("Maximum tree depth",
#                        breaks = -3:-1, labels = c(0.001, 0.01, 0.1)) +
#     facet_grid(gsize ~ ., scales = "free") +
#     theme(axis.title = element_text(size = 11),
...
save_fig("Fig_4.pdf", pip_plot, width = 169, height = 90)
```
...
After downloading, I read the compressed FASTA file and filtered out
scaffolds by using a size threshold
...
variant haplotypes from the reference genome (object `ssites`).
Twenty of the haplotypes are from one population, twenty from another.
...
`haps_ssites` function that checks and organizes the coalescent output object,
I created haplotypes from the reference genome:
...
In this case, each individual haplotype's reads were output to separate FASTQ files.
I also wrote the true mutation information to a VCF file.
...
First, I simulated a random species tree of 10 species (object `tree`) using
`ape::rcoal(8)` [@Paradis_2018].
...
`24 1 -T -r 1 C -I 3 8 8 8 0 -ej 1.0 3 1 -ej 0.5 2 1` for a chromosome size of `C`), 
...
```{r examples-phylogeny-gtrees-create-haplotypes, echo = FALSE, eval = TRUE}
# Takes ~1.5 min
...
haplotype_barcodes <- rep(c("TCGCCTTA", "CTAGTACG", "TTCTGCCT", "GCTCAGGA", "AGGAGTCC", 
                          ...
                          "TGCCTCTT", "TCCTCTAC"), 2)
```
```{r examples-phylogeny-gtrees-create-haplotypes-for-show}
haps <- create_haplotypes(ref, haps_gtrees(gtrees), sub, ins, del)
```
...
I also wrote the true gene trees to an `ms`-style file.
...
simulations using `jackalope` are simpler to implement.
They are also more efficient computationally, but users should weight this performance
...
`jackalope` should inform research design for projects employing HTS,
...
```{r haplotypes-perf-test-plot, echo = FALSE, eval = TRUE, fig.width=112/25.4, fig.height=169/25.4, fig.cap=hap_plot_cap}
hap_p
```
